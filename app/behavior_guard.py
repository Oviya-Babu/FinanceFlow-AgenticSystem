...
class BehaviorGuard:
    ...
    async def get_metrics(self) -> Dict:
        ...
        async with self.lock:
            # Count suspicious directly without calling is_suspicious()
            # to avoid trying to acquire the lock again
            suspicious = set()
            for agent_id, profile in self.profiles.items():
                if profile.request_count >= self.min_requests_for_detection:
                    # Inline the suspicious detection logic
                    is_suspicious = False
                    ...
                    if profile.get_failure_rate() > self.failure_rate_threshold:
                        is_suspicious = True
                    # Check block rate
                    elif profile.get_block_rate() > self.block_rate_threshold:
                        is_suspicious = True
            ...
            return {
                ...
                "suspicious_agents_total": len(suspicious),
            }
