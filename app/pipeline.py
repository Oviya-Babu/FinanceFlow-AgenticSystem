...
async def step_4_rbac_check(
    ...
    Enhanced to support:
    - Tool context evaluation (e.g., read_pdf + public_docs YES, internal_files NO)
    - Decision traceability (all rules evaluated, decision trace returned)
    - Velocity limit checks (per_sec, per_min, per_hour)
    ...
    try:
        ...
        # ====================================================================
        # ENHANCED: Build policy input with context, metadata, velocity info
        # ====================================================================
        policy_input = {
            "agent_id": context.agent_id,
            "agent_role": agent_role,
            "tool_name": context.tool_name,
            # Tool context: e.g., "public_documents", "admin_db", "internal_team"
            "tool_context": getattr(context, "tool_context", None),
            # Velocity metrics (simulated; real implementation would query Redis)
            "current_call_count_per_sec": 0,
            "current_call_count_per_min": 0,
            "current_call_count_per_hour": 0,
            # Request metadata
            "request_id": context.request_id,
            "timestamp": time.time(),
        }
        ...
        async with httpx.AsyncClient() as client:
            ...
            # ================================================================
            # ENHANCED: Capture decision trace for debugging and audit
            # ================================================================
            decision_trace = result.get("decision_trace", [])
            velocity_status = result.get("velocity_status", "unknown")
            ...
            if not allowed:
                logger.warning(
                    ...
                    extra={
                        ...
                        "reason": context_reason or "policy denied access",
                    },
                )
                raise RBACDeniedException(
            ...
            # Log successful decisions with trace
            logger.info(
                "RBAC check allowed",
                extra={
                    "agent_id": context.agent_id,
